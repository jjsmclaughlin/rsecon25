...
def main(trained_pipeline: Path, test_data: Path, print_details: bool):
    ...
    #egline = 'Scar on top of head, forehead and back of left hand, lost four teeth, mole top of left shoulder, lost left arm.'
    #print(egline)
    ...
    for doc in docs:
        strdocs.append(str(doc))

    random.shuffle(strdocs)

    for strdoc in strdocs:
        ...
        for ent in doc.ents:
            print(str(ent.start) + ' : ' + ent.text + ' : ' + ent.label_)
...
#
#            for value, rel_dict in pred._.rel.items():
#                if (rel_dict['Injurybody'] > 0.5):
#                    print(str(value) + ' : ' + str(rel_dict))
#                #gold_labels = [k for (k, v) in gold._.rel[value].items() if v == 1.0]
#                #if gold_labels:
#                #    print(
#                #        f" pair: {value} --> gold labels: {gold_labels} --> predicted values: {rel_dict}"
